...
class addmapmission:
    ...
    def POST(self):
        ...
        try:
            ...
            contour = i.get('contour')
            linecolor = i.get('linecolor')
            barb = i.get('barb')
            f = open('waitlistmission.sh', 'a+')
            f.write('python3 main.py --time ' + time + ' --type map --contourf ' + contourf + ' --contourfcolor ' + contourfcolor + ' --contour ' + contour + ' --barb ' + barb + ' --contourcolor ' + linecolor + ' --area ' + area)
            print('!![PRCOESS]!!')
            f.close()
            return web.redirect('success')
        except:
